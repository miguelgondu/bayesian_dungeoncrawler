...
class ZeldaExperiment:
    ...
    """
    def __init__(self, path, goal, behaviors=[], times=[], projection=None, verbose=True, model_parameters=None):
        self.path = path
        ...
        if model_parameters is None:
            ...
            self.model_parameters = default_mp
            ...
            if self.verbose:
                ...
                for name, flag in self.model_parameters.items():
                    if flag:
                        print(name, end=" ")
                    print()
            
            self.kernel = sum([
                kernel for name, kernel in kernels.items() if self.model_parameters[name]
            ])
    ...
    def save_3D_plot(self, save_path, plot_sigma=True, plot_dots=True):
        ...
        if len(self.behaviors) > 0 and plot_dots:
